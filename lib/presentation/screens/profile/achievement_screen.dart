...
// Load live counters from the real activity tables.
// This keeps Achievement progress correct even when an older
// user_achievements.progress_value row was not refreshed by another page.
      final Map<String, int> liveProgress =
      await _loadLiveProgressCounters(user.id);
...
// Prefer live activity data for the requirement types that can be
// derived safely from this user's real Supabase records. Unknown
// requirement types keep the stored user_achievements progress as fallback.
        final String normalizedRequirement =
        _normalizeRequirementType(requirementType);

        if (liveProgress.containsKey(normalizedRequirement)) {
          currentProgress = liveProgress[normalizedRequirement] ?? 0;
        }
...
// the whole Achievement page.
  ...
  Future<Map<String, int>> _loadLiveProgressCounters(
      String userId,
      ) async {
    int missionCount = 0;
...
// Completed checkpoint missions.
    try {
      ...
      final List<String> missionIds = completedMissions
          .map((dynamic row) => row is Map ? row['mission_id']?.toString() : null)
          .whereType<String>()
      ...
      if (missionIds.isNotEmpty) {
        final List<dynamic> missionDefinitions =
        await SupabaseConfig.client
            .from('checkpoint_missions')
            .select('mission_id, destination_id')
            .inFilter('mission_id', missionIds);

        checkpointCount = missionDefinitions
            .map((dynamic row) =>
        row is Map ? row['destination_id']?.toString() : null)
            .whereType<String>()
            .where((String id) => id.isNotEmpty)
            .toSet()
            .length;
      ...
      debugPrint('ACHIEVEMENT LIVE MISSION LOAD ERROR: $error');
    }

// Completed puzzle rounds.
    try {
      ...
      debugPrint('ACHIEVEMENT LIVE PUZZLE LOAD ERROR: $error');
    }

// Successful Blind Box draws recorded in history.
    try {
      final List<dynamic> blindBoxRows =
      await SupabaseConfig.client
          .from('blind_box_history')
          .select('history_id')
          .eq('user_id', userId);

      blindBoxCount = blindBoxRows.length;
    } catch (error) {
      debugPrint('ACHIEVEMENT LIVE BLIND BOX LOAD ERROR: $error');
    }

    return <String, int>{
      'exploration_points': _explorationPoints,
      'mission_count': missionCount,
      'checkpoint_count': checkpointCount,
      'puzzle_count': puzzleCount,
      'blind_box_count': blindBoxCount,
    };
  }

  String _normalizeRequirementType(String value) {
    final String key = value.trim().toLowerCase().replaceAll('-', '_');

    switch (key) {
      case 'exploration point':
      case 'exploration points':
      case 'points':
      case 'ep':
      ...
      case 'checkpoints_completed':
      case 'completed_checkpoints':
      ...
      case 'blind_boxes':
      case 'blind boxes':
      case 'blind box count':
      case 'blindbox_count':
      case 'blind_boxes_completed':
        return 'blind_box_count';
      default:
        return key;
    }
  }
  ...
  List<AchievementViewData> get _inProgressItems {
    final List<AchievementViewData> items = _achievements
        .where((achievement) => achievement.status == 'in_progress')
        .toList();

    items.sort((a, b) => a.targetValue.compareTo(b.targetValue));
    return items;
  }

  List<AchievementViewData> get _allAchievements {
    final List<AchievementViewData> items = _achievements
        .where(
          (achievement) =>
  ...
  int get _totalBadgeCount => _achievements
      .where((item) => item.achievementType.toUpperCase() == 'BADGE')
      .length;

  int get _unlockedBadgeCount => _achievements
      .where((item) =>
  item.achievementType.toUpperCase() == 'BADGE' &&
      item.status == 'unlocked')
      .length;

  List<AchievementViewData> get _allBadges {
    final List<AchievementViewData> items = _achievements
        .where(
          (achievement) =>
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (_errorMessage != null)
                      ...
                      if (_inProgressItems.isNotEmpty) ...[
                        const Text(
                          'In Progress',
                          style: TextStyle(
                            color: darkText,
                        ...
                        const Text(
                          'KEEP EXPLORING TO COMPLETE THESE ACHIEVEMENTS AND BADGES',
                          style: TextStyle(
                            color: primaryBlue,
                            fontSize: 8.5,
                            fontWeight: FontWeight.w900,
                      ...
                      Row(
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(
                              '$_unlockedBadgeCount/$_totalBadgeCount',
...
// No achievement image URL is hardcoded in this Dart file.
        if (imageUrl != null)
          ColorFiltered(
// Locked achievements are shown in true grayscale.
// In-progress and unlocked achievements keep the original
// full-colour image from Supabase.
            colorFilter: achievement.status == 'locked'
          ...
          ColorFiltered(
            ...
            child: _achievementFallbackBackground(
              achievement,
              themeColors,
            ),
          ),
...
// Locked items are grayscale above and receive a slightly darker
// overlay so they are immediately distinguishable from unlocked cards.
